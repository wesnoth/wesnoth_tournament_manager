...
-- 1. Update users table to include timezone and availability
ALTER TABLE tournament.users
...
CREATE TABLE IF NOT EXISTS tournament.match_schedule_proposals (
  ...
  proposed_by_user_id CHAR(36) NOT NULL COMMENT 'FK→users.id',
  ...
  FOREIGN KEY (proposed_by_user_id) REFERENCES tournament.users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS tournament.match_schedule_slots (
  id CHAR(36) PRIMARY KEY COMMENT 'UUID v4',
  ...
  FOREIGN KEY (proposal_id) REFERENCES tournament.match_schedule_proposals(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS tournament.match_schedule_confirmations (
  id CHAR(36) PRIMARY KEY COMMENT 'UUID v4',
  ...
  FOREIGN KEY (slot_id) REFERENCES tournament.match_schedule_slots(id) ON DELETE CASCADE,
  FOREIGN KEY (user_id) REFERENCES tournament.users(id) ON DELETE CASCADE,
  FOREIGN KEY (team_id) REFERENCES tournament.tournament_teams(id) ON DELETE SET NULL,
