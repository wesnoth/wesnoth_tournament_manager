...
export interface AuditLogEntry {
  event_type: 'LOGIN_SUCCESS' | 'LOGIN_FAILED' | 'REGISTRATION' | 'ADMIN_ACTION' | 'SECURITY_EVENT' | 'PASSWORD_RESET_REQUEST' | 'EMAIL_VERIFIED' | 'ACCOUNT_UNLOCKED' | 'PASSWORD_RESET' | 'MAINTENANCE_MODE_TOGGLE' | 'PROFILE_UPDATE' | 'USER_BLOCKED' | 'USER_UNBLOCKED' | 'REPLAY_FORCE_DISCARDED' | 'REPLAY_AUTO_DISCARDED' | 'REPLAY_REPROCESS_REQUESTED' | 'PARTICIPANT_REMOVED' | 'TEAM_RENAMED';
  user_id?: string;
  username?: string;
  ip_address?: string;
  user_agent?: string;
