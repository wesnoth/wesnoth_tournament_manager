...
/** One row = one faction as Side 1 vs another as Side 2, aggregated across all maps */
interface SideFactionRow {
...
/**
 * Convert /matchups rows (one row per map per faction-pair, faction_id < opponent_id)
 * into per-side rows aggregated across all maps.
 */
function globalToSideFactionRows(data: any[], minGames: number): SideFactionRow[] {
  const accum = new Map<string, SideFactionAccum>();
  ...
  return Array.from(accum.values())
    ...
    .map(r => {
      const wr = r.games > 0 ? (r.side1_wins / r.games) * 100 : 0;
      return { ...r, side1_winrate: wr, imbalance: Math.abs(wr - 50) * 2 };
    })
    .sort((a, b) => b.imbalance - a.imbalance);
}

/**
 * Convert balance event comparison data into per-side rows (no map dimension).
 * Each match appears twice (once per faction perspective) → deduplicate with Map.
 */
function comparisonToSideFactionRows(data: any[]): SideFactionRow[] {
...
const FactionVsFactionTab: React.FC<{ beforeData?: any[] | null; afterData?: any[] | null }> = ({
  ...
  const [stats, setStats]             = useState<SideFactionRow[]>([]);
  const [beforeStats, setBeforeStats] = useState<SideFactionRow[]>([]);
  const [afterStats, setAfterStats]   = useState<SideFactionRow[]>([]);
  ...
  useEffect(() => {
    ...
    statisticsService.getMatchupStats(minGames)
      .then(data => setStats(globalToSideFactionRows(data, minGames)))
  ...
  if (showComparison) {
    const allKeys = new Set([
      ...beforeStats.map(r => `${r.side1_faction_id}|${r.side2_faction_id}`),
      ...afterStats.map(r  => `${r.side1_faction_id}|${r.side2_faction_id}`),
    ]);
    const beforeMap = new Map(beforeStats.map(r => [`${r.side1_faction_id}|${r.side2_faction_id}`, r]));
    const afterMap  = new Map(afterStats.map(r  => [`${r.side1_faction_id}|${r.side2_faction_id}`, r]));
    ...
    return (
      <div className="bg-white rounded-lg p-6 shadow-md">
        <h3 className="text-xl font-semibold text-gray-800 mb-3">{t('faction_vs_faction_comparison') || 'Faction vs Faction — Before & After'}</h3>
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-md">
      <h3 className="text-xl font-semibold text-gray-800 mb-3">{t('faction_vs_faction') || 'Faction vs Faction'}</h3>
