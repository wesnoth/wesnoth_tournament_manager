...
const PlayerStatsByMatchup: React.FC<Props> = ({ playerId }) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h3 className="text-2xl font-semibold text-gray-800 mb-4">{t('matchup_analysis') || 'Matchup Analysis'}</h3>
      <p className="text-gray-600 text-sm mb-6">{t('matchup_analysis_explanation') || 'Your performance against each faction matchup'}</p>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-x-auto">
          <table className="w-full border-collapse bg-white">
            <thead className="bg-gray-100 border-b-2 border-gray-300">
              <tr>
                <th className="px-4 py-3 text-left font-semibold text-gray-700">{t('your_faction') || 'Your Faction'}</th>
                <th className="px-4 py-3 text-left font-semibold text-gray-700">{t('opponent_faction') || 'Opponent Faction'}</th>
            ...
            <tbody>
              {stats.map((stat) => (
                <tr key={`${stat.faction_id}-${stat.opponent_faction_id}`} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="px-4 py-3 text-left font-semibold text-gray-700">{stat.faction_name}</td>
                  <td className="px-4 py-3 text-left font-semibold text-gray-700">{stat.opponent_faction_name}</td>
...
export default PlayerStatsByMatchup;
